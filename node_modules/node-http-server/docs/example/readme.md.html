    ...
    <ul class="sectionDetails docsOnly">
        ...
        <li id="section-1">
            <div class="annotation">
                <h2 id="node-http-and-https-server-examples">Node HTTP and HTTPS Server examples</h2>
<p>All of the http example servers spin up on port 8000 so you can only run one at a time.</p>
<p>And All of the https examples 4433 so you can only run one at a time, it is also worth noting that many of the https examples spin up both an http AND an https server. So both 8000 and 4433 will be used. This is for demonstration purposes and you can specify only to spin up a secure server with <code>config.https.only=true</code> or by setting it in your config object;</p>
<p>The servers in the <a href="https://github.com/RIAEvangelist/node-http-server/tree/master/example/basic">basic server folder</a> are just static file servers, and the servers in the <a href="https://github.com/RIAEvangelist/node-http-server/tree/master/example">advanced server folder</a> are dynamic server side rendering examples.</p>
<p>You can see detailed code documentation of the examples on the <a href="http://riaevangelist.github.io/node-http-server/">node-http-server docs</a> under the examples list.</p>

...
<html>
<head>
    <title>https-and-http-google-proxy</title>
...
<h1>
    https-and-http-google-proxy
</h1>
<div class="subHeading">
    ./example/proxy/https-and-http-google-proxy.js
</div>
    ...
    <ul class="sectionDetails ">
        ...
        <li id="section-1">
            <div class="annotation">
                ...
                <div class="highlight"><pre><span class="hljs-keyword">const</span> util = <span class="hljs-built_in">require</span>( <span class="hljs-string">'util'</span> );</pre></div>
            </div>
        </li>
...
<code>const server=require(‘node-http-server’);</code></p>
                ...
                <div class="highlight"><pre><span class="hljs-keyword">const</span> server=<span class="hljs-built_in">require</span>(<span class="hljs-string">'../../server/Server.js'</span>);</pre></div>
            </div>
        </li>
        ...
        <li id="section-3">
            <div class="annotation">
                <p>I am using request for simplicty sake here, you can too.</p>

            </div><div class="content">
                <div class="highlight"><pre><span class="hljs-keyword">const</span> proxy=<span class="hljs-built_in">require</span>(<span class="hljs-string">'request'</span>);
...
config.https.certificate= <span class="hljs-string">`<span class="hljs-subst">${__dirname}</span>/../../local-certs/client.crt`</span>;
config.https.ca= <span class="hljs-string">`<span class="hljs-subst">${__dirname}</span>/../../local-certs/private/rootCA.pem`</span>;
...
config.https.port       = <span class="hljs-number">4433</span>;</pre></div>
        ...
        <li id="section-4">
            <div class="annotation">
                <p>lets ignore ssl issues and make a giant security hole since we are proxying https too…
be careful when proxying ssl. you should actually set your ca and certs properly.
this is just an example</p>

            </div><div class="content">
                <div class="highlight"><pre>process.env.NODE_TLS_REJECT_UNAUTHORIZED = <span class="hljs-number">0</span>;

<span class="hljs-function"><span class="hljs-keyword">function</span> <span class="hljs-title">gotRequest</span>(<span class="hljs-params">request,response,serve</span>)</span>{</pre></div>
            </div>
        </li>
        ...
        <li id="section-5">
            <div class="annotation">
                <p>google proxy!
handle things fast</p>

            </div><div class="content">
                <div class="highlight"><pre>    <span class="hljs-keyword">let</span> encoding=<span class="hljs-string">'binary'</span>;

    proxy(
        {
            <span class="hljs-attr">url</span>: <span class="hljs-string">`<span class="hljs-subst">${request.uri.protocol}</span>://www.google.com<span class="hljs-subst">${request.uri.path}</span><span class="hljs-subst">${request.uri.search}</span>`</span>,
            <span class="hljs-attr">encoding</span>:encoding
        },
        <span class="hljs-function"><span class="hljs-keyword">function</span> (<span class="hljs-params">error, proxiedResponse, proxiedBody</span>) </span>{
            <span class="hljs-keyword">if</span> (error) {
                request.statusCode=<span class="hljs-number">500</span>;
                serve(request,response,<span class="hljs-built_in">JSON</span>.stringify(error));
                <span class="hljs-keyword">return</span>;
            }

            <span class="hljs-keyword">if</span>(!proxiedResponse.headers[<span class="hljs-string">'content-type'</span>]&amp;&amp;proxiedResponse.headers[<span class="hljs-string">'Content-Type'</span>]){
                proxiedResponse.headers[<span class="hljs-string">'content-type'</span>]=proxiedResponse.headers[<span class="hljs-string">'Content-Type'</span>];
            }

            <span class="hljs-keyword">if</span>(!proxiedResponse.headers[<span class="hljs-string">'content-type'</span>]){
                proxiedResponse.headers[<span class="hljs-string">'content-type'</span>]=<span class="hljs-string">''</span>;
            }

            <span class="hljs-keyword">if</span>(proxiedResponse.headers[<span class="hljs-string">'content-type'</span>].indexOf(<span class="hljs-string">'text/html'</span>)&gt;<span class="hljs-number">-1</span>){
                <span class="hljs-keyword">const</span> position=proxiedBody.match(<span class="hljs-regexp">/&lt;body([^&gt;]*)&gt;/i</span>);

                <span class="hljs-keyword">if</span>(position&amp;&amp;position.index){
                    proxiedBody=proxiedBody.slice(<span class="hljs-number">0</span>, position.index+position[<span class="hljs-number">0</span>].length) + <span class="hljs-string">`
                        &lt;style&gt;
                            .proxyBanner{
                                height:5em;
                    ...
                    `</span>+ proxiedBody.slice(position.index+position[<span class="hljs-number">0</span>].length)
                }
            }
...
server.onRawRequest=gotRequest;</pre></div>
            </div>
        </li>

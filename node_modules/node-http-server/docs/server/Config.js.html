...
<span class="hljs-keyword">const</span> passedArgs = process.argv.splice(<span class="hljs-number">2</span>),
...
<span class="hljs-keyword">const</span> defaults = {
    ...
    <span class="hljs-attr">domain</span>  : <span class="hljs-string">'0.0.0.0'</span>,
...
<span class="hljs-keyword">for</span>(<span class="hljs-keyword">let</span> i=<span class="hljs-number">0</span>; i&lt;argCount; i++){
    <span class="hljs-keyword">const</span> data=passedArgs[i].split(<span class="hljs-string">'='</span>);
    args[data[<span class="hljs-number">0</span>]]=data[<span class="hljs-number">1</span>];
}</pre></div>
            </div>
        </li>
...
<p> <a href="https://github.com/RIAEvangelist/node-http-server/#config-class">Detailed default config docs and explination</a></p>

            </div><div class="content">
                <div class="highlight"><pre><span class="hljs-class"><span class="hljs-keyword">class</span> <span class="hljs-title">Config</span></span>{
    <span class="hljs-keyword">constructor</span>(userConfig){
      <span class="hljs-built_in">Object</span>.assign(<span class="hljs-keyword">this</span>,defaultConfigs);

      <span class="hljs-keyword">if</span>(userConfig){
          <span class="hljs-keyword">for</span>(<span class="hljs-keyword">const</span> k <span class="hljs-keyword">in</span> userConfig){
              <span class="hljs-keyword">this</span>[k]=userConfig[k];
          }
      }
    }
}</pre></div>
        ...
        <li id="section-3">
            <div class="annotation">
                <h3 id="default-node-http-server-config-values">Default node HTTP server config values</h3>
...
<p> <a href="https://github.com/RIAEvangelist/node-http-server/#default-node-http-server-configuration">Detailed default config docs and explination</a></p>

            </div><div class="content">
                <div class="highlight"><pre><span class="hljs-keyword">const</span> defaultConfigs={
    ...
    <span class="hljs-attr">https</span>:{
        ...
        <span class="hljs-attr">passphrase</span>:<span class="hljs-literal">false</span>,
...
<span class="hljs-function"><span class="hljs-keyword">function</span> <span class="hljs-title">serverLogging</span>(<span class="hljs-params">data</span>)</span>{
    fs.exists(
        <span class="hljs-keyword">this</span>.log,
        <span class="hljs-function"><span class="hljs-keyword">function</span> <span class="hljs-title">serverLogExsits</span>(<span class="hljs-params">exists</span>)</span>{
            data.timestamp=<span class="hljs-keyword">new</span> <span class="hljs-built_in">Date</span>().getTime();

            <span class="hljs-keyword">const</span> JSONData=<span class="hljs-built_in">JSON</span>.stringify(data);
            <span class="hljs-keyword">let</span> method=<span class="hljs-string">'appendFile'</span>;
            <span class="hljs-keyword">if</span>(!exists){
                method=<span class="hljs-string">'writeFile'</span>;
            }
            fs[method](
                <span class="hljs-keyword">this</span>.log,
                JSONData,
                <span class="hljs-function"><span class="hljs-keyword">function</span> <span class="hljs-title">fsMethod</span>(<span class="hljs-params">err</span>) </span>{
...
<span class="hljs-built_in">module</span>.exports=Config;</pre></div>
            </div>
        </li>
